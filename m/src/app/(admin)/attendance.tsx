...
export default function AttendanceScreen() {
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      await attendanceApi.update(editItem._id || editItem.id, {
        studentId: form.studentId.trim(),
        tripId: form.tripId.trim(),
        status: form.status,
        checkInMethod: form.checkInMethod,
  ...
  const handleDelete = (item: any) => {
    Alert.alert('حذف سجل', `هل أنت متأكد من حذف سجل الحضور này؟\nلا يمكن التراجع عن هذا الإجراء.`, [
      { text: 'إلغاء', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await attendanceApi.delete(item._id || item.id);
            refetch();
            Alert.alert('تم', 'تم حذف سجل الحضور بنجاح');
          } catch (err: any) {
            Alert.alert('خطأ', err?.response?.data?.message || 'فشل حذف سجل الحضور');
          }
        },
      },
    ]);
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: spacing.md }}>
              {(students ?? []).map((s: any) => {
                ...
                return (
                  <TouchableOpacity key={id} style={[styles.chip, selected && styles.chipSelected]} onPress={() => setField('studentId', id)}>
                    <Text style={[styles.chipText, selected && styles.chipTextSelected]}>{s.fullName || id}</Text>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: spacing.md }}>
              {(trips ?? []).map((t: any) => {
                ...
                return (
                  <TouchableOpacity key={id} style={[styles.chip, selected && styles.chipSelected]} onPress={() => setField('tripId', id)}>
                    <Text style={[styles.chipText, selected && styles.chipTextSelected]}>{t.tripNumber || id}</Text>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: spacing.md }}>
              {methodOptions.map((m) => {
                ...
                return (
                  <TouchableOpacity key={m.value} style={[styles.chip, selected && styles.chipSelected]} onPress={() => setField('checkInMethod', m.value)}>
                  ...
                  <View style={styles.cardHeader}>
                    ...
                    <View style={{ flex: 1, marginHorizontal: spacing.md }}>
                      ...
                      <Text style={[typography.bodySmall, { color: colors.textSecondary }]}>
                        الرحلة: {item.tripId?.tripNumber || '--'}
                      </Text>
                      <Text style={[typography.bodySmall, { color: colors.textTertiary }]}>
                        {item.checkInTime ? new Date(item.checkInTime).toLocaleTimeString('ar-SA') : '--'} | {item.checkInMethod === 'qr' ? 'QR' : item.checkInMethod === 'gps' ? 'GPS' : 'يدوي'}
                      </Text>
                    </View>
