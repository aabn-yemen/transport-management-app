...
const emptyForm = { destinationName: '', city: '', description: '', status: 'active' };

export default function DestinationsScreen() {
  const { width, isSmall, isMedium, isLarge, isXLarge, contentMaxWidth, modalWidth, horizontalPadding } = useResponsive();
  const [search, setSearch] = useState('');
  const { data: destinations, isLoading, refetch } = useGetAll<any[]>(['destinations'], destinationsApi.getAll);
  ...
  const filtered = (destinations ?? []).filter((d: any) =>
    !search || d.destinationName?.includes(search) || d.city?.includes(search)
  );
  ...
  const handleAdd = async () => {
    if (!form.destinationName.trim() || !form.city.trim()) {
    ...
    try {
      ...
      setForm(emptyForm);
      refetch();
      Alert.alert('تم', 'تمت إضافة الوجهة بنجاح');
    } catch (err: any) {
      Alert.alert('خطأ', err?.response?.data?.message || 'فشل إضافة الوجهة');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      await destinationsApi.update(editItem._id || editItem.id, {
        destinationName: form.destinationName.trim(),
        city: form.city.trim(),
        description: form.description.trim() || undefined,
        status: form.status,
      ...
      setForm(emptyForm);
      refetch();
      Alert.alert('تم', 'تم تعديل بيانات الوجهة بنجاح');
    } catch (err: any) {
      Alert.alert('خطأ', err?.response?.data?.message || 'فشل تعديل بيانات الوجهة');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = (item: any) => {
    Alert.alert('حذف وجهة', `هل أنت متأكد من حذف "${item.destinationName}"؟\nلا يمكن التراجع عن هذا الإجراء.`, [
      { text: 'إلغاء', style: 'cancel' },
      {
        text: 'حذف', style: 'destructive',
        onPress: async () => {
          try {
            await destinationsApi.delete(item._id || item.id);
            refetch();
            Alert.alert('تم', 'تم حذف الوجهة بنجاح');
          } catch (err: any) {
            Alert.alert('خطأ', err?.response?.data?.message || 'فشل حذف الوجهة');
          }
        },
      },
    ]);
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
        ...
        <View style={[styles.modalContent, { width: modalWidth, alignSelf: 'center' }]}>
          ...
          <Text style={styles.label}>اسم الوجهة *</Text>
          <TextInput
            style={styles.modalInput} value={form.destinationName} onChangeText={(v) => setField('destinationName', v)}
            placeholder="اسم الوجهة" placeholderTextColor={colors.textTertiary} textAlign="right"
          />

          <Text style={styles.label}>المدينة *</Text>
          <TextInput
            style={styles.modalInput} value={form.city} onChangeText={(v) => setField('city', v)}
            placeholder="المدينة" placeholderTextColor={colors.textTertiary} textAlign="right"
          />

          <Text style={styles.label}>الوصف</Text>
          <TextInput
            style={styles.modalInput} value={form.description} onChangeText={(v) => setField('description', v)}
            placeholder="وصف الوجهة (اختياري)" placeholderTextColor={colors.textTertiary} textAlign="right"
          />
  ...
  return (
    <View style={styles.container}>
        ...
        >
          {filtered.length === 0 ? (
            <View style={{ alignItems: 'center', marginTop: spacing.xxxxl }}>
              ...
              <Text style={[typography.bodyMedium, { color: colors.textSecondary, textAlign: 'center', marginTop: spacing.md }]}>لا يوجد وجهات</Text>
            </View>
          ) : (
              ...
              <Card key={item._id ?? item.id} style={styles.card}>
                <View style={styles.row}>
                  ...
                  <View style={{ flex: 1, marginHorizontal: spacing.md }}>
                    <Text style={[typography.titleSmall, { color: colors.text }]}>{item.destinationName}</Text>
                    <Text style={[typography.bodySmall, { color: colors.textSecondary }]}>المدينة: {item.city}</Text>
                    {item.description ? (
      ...
      {renderFormModal(addVisible, () => setAddVisible(false), handleAdd, 'إضافة وجهة جديدة')}
      {renderFormModal(editVisible, () => { setEditVisible(false); setEditItem(null); }, handleEdit, 'تعديل بيانات الوجهة')}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: colors.success + '15', justifyContent: 'center', alignItems: 'center' },
