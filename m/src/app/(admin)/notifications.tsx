...
const emptyForm = { title: '', body: '', type: 'info' };
...
export default function NotificationsScreen() {
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      ...
      refetch();
      Alert.alert('تم', 'تم تعليم جميع الإشعارات كمقروءة');
    } catch (err: any) {
      Alert.alert('خطأ', err?.response?.data?.message || 'فشل تعليم الإشعارات');
    } finally {
      setSaving(false);
    }
  };
      ...
      { text: 'إلغاء', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert('خطأ', err?.response?.data?.message || 'فشل حذف الإشعار');
          }
        },
      },
    ]);
          ...
          <TouchableOpacity style={[styles.markAllBtn, saving && { opacity: 0.6 }]} onPress={handleMarkAllRead} disabled={saving}>
            {saving ? (
              ...
              <>
                ...
                <Text style={[typography.button, { color: colors.textInverse, marginLeft: 6 }]}>تعليم الكل كمقروء ({unreadCount})</Text>
              </>
            )}
          </TouchableOpacity>
        )}
                      ...
                      <View style={{ flex: 1, marginHorizontal: spacing.md }}>
                        ...
                        <View style={styles.bottomRow}>
                          ...
                          <View style={[styles.typeBadge, { backgroundColor: cfg.color + '15' }]}>
                            <Text style={[typography.bodySmall, { color: cfg.color, fontSize: 10 }]}>{cfg.label}</Text>
                          </View>
                        </View>
                      </View>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
          ...
          <View style={[styles.modalContent, { width: modalWidth }]}>
            {selectedItem && (() => {
              ...
              return (
                <>
                  <View style={[styles.detailIcon, { backgroundColor: cfg.color + '15' }]}>
                  ...
                  <Text style={[typography.bodySmall, { color: colors.textTertiary, textAlign: 'center', marginTop: spacing.md }]}>
                    {new Date(selectedItem.createdAt).toLocaleDateString('ar-SA', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </Text>
                  <TouchableOpacity style={styles.closeBtn} onPress={() => setDetailVisible(false)}>
                    <Text style={[typography.button, { color: colors.textInverse }]}>إغلاق</Text>
                  </TouchableOpacity>
                </>
              );
            })()}
