...
export default function SubscriptionsScreen() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setForm(emptyForm);
      refetch();
      Alert.alert('تم', 'تمت إضافة الاشتراك بنجاح');
    } catch (err: any) {
      Alert.alert('خطأ', err?.response?.data?.message || 'فشل إضافة الاشتراك');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      await subscriptionsApi.update(editItem._id || editItem.id, {
        studentId: form.studentId.trim(),
        busId: form.busId.trim(),
        startDate: form.startDate.trim(),
        endDate: form.endDate.trim(),
      ...
      setForm(emptyForm);
      refetch();
      Alert.alert('تم', 'تم تعديل بيانات الاشتراك بنجاح');
    } catch (err: any) {
      Alert.alert('خطأ', err?.response?.data?.message || 'فشل تعديل بيانات الاشتراك');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = (item: any) => {
    Alert.alert('حذف اشتراك', `هل أنت متأكد من حذف هذا الاشتراك؟\nلا يمكن التراجع عن هذا الإجراء.`, [
      { text: 'إلغاء', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await subscriptionsApi.delete(item._id || item.id);
            refetch();
            Alert.alert('تم', 'تم حذف الاشتراك بنجاح');
          } catch (err: any) {
            Alert.alert('خطأ', err?.response?.data?.message || 'فشل حذف الاشتراك');
          }
        },
      },
    ]);
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
        ...
        <View style={[styles.modalContent, { width: modalWidth, alignSelf: 'center' }]}>
          ...
          <TextInput
            style={styles.modalInput} value={form.studentId} onChangeText={(v) => setField('studentId', v)}
            placeholder="معرف الطالب" placeholderTextColor={colors.textTertiary} textAlign="right"
          />
          ...
          <TextInput
            style={styles.modalInput} value={form.startDate} onChangeText={(v) => setField('startDate', v)}
          ...
          <TextInput
            style={styles.modalInput} value={form.endDate} onChangeText={(v) => setField('endDate', v)}
            placeholder="YYYY-MM-DD" placeholderTextColor={colors.textTertiary} textAlign="right"
          />

          <Text style={styles.label}>المبلغ *</Text>
          <TextInput
            style={styles.modalInput} value={form.amount} onChangeText={(v) => setField('amount', v)}
          ...
          <TextInput
            style={styles.modalInput} value={form.discount} onChangeText={(v) => setField('discount', v)}
            placeholder="0" placeholderTextColor={colors.textTertiary} textAlign="right" keyboardType="numeric"
          />
          ...
          <View style={styles.chipRow}>
            {['pending', 'paid', 'partial'].map((s) => (
              <TouchableOpacity
                key={s}
                style={[styles.chip, form.paymentStatus === s && styles.chipActive]}
                onPress={() => setField('paymentStatus', s)}
              >
                <Text style={[typography.bodySmall, { color: form.paymentStatus === s ? colors.textInverse : colors.textSecondary }]}>
                  {s === 'pending' ? 'قيد الانتظار' : s === 'paid' ? 'مدفوع' : 'جزئي'}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
              ...
              return (
                <Card key={item._id ?? item.id} style={styles.card}>
                  <View style={styles.cardHeader}>
                    <View style={{ flex: 1 }}>
                      <Text style={[typography.titleSmall, { color: colors.text }]}>
                        {item.studentId?.fullName || item.studentId || 'طالب'}
                      </Text>
                    ...
                    <View style={[styles.badge, { backgroundColor: sBadge.bg }]}>
                      <Text style={[typography.bodySmall, { color: sBadge.fg }]}>{sBadge.label}</Text>
                    </View>
                  </View>
                  ...
                  <View style={styles.cardFooter}>
                    <View style={[styles.badge, { backgroundColor: pBadge.bg }]}>
                      <Text style={[typography.bodySmall, { color: pBadge.fg }]}>{pBadge.label}</Text>
                    </View>
