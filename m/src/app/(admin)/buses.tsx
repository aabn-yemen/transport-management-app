...
export default function BusesScreen() {
  ...
  const handleAdd = async () => {
    if (!form.busNumber.trim() || !form.plateNumber.trim()) {
    ...
    try {
      ...
      setForm(emptyForm);
      refetch();
      Alert.alert('تم', 'تمت إضافة الحافلة بنجاح');
    } catch (err: any) {
      Alert.alert('خطأ', err?.response?.data?.message || 'فشل إضافة الحافلة');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      await busesApi.update(editItem._id || editItem.id, {
        busNumber: form.busNumber.trim(),
        plateNumber: form.plateNumber.trim(),
        brand: form.brand.trim() || undefined,
        modelName: form.modelName.trim() || undefined,
      ...
      setForm(emptyForm);
      refetch();
      Alert.alert('تم', 'تم تعديل بيانات الحافلة بنجاح');
    } catch (err: any) {
      Alert.alert('خطأ', err?.response?.data?.message || 'فشل تعديل بيانات الحافلة');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = (item: any) => {
    Alert.alert('حذف حافلة', `هل أنت متأكد من حذف حافلة "${item.busNumber}"؟\nلا يمكن التراجع عن هذا الإجراء.`, [
      { text: 'إلغاء', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await busesApi.delete(item._id || item.id);
            refetch();
            Alert.alert('تم', 'تم حذف الحافلة بنجاح');
          } catch (err: any) {
            Alert.alert('خطأ', err?.response?.data?.message || 'فشل حذف الحافلة');
          }
        },
      },
    ]);
        ...
        <ScrollView>
          <View style={[styles.modalContent, { width: modalWidth, alignSelf: 'center' }]}>
            ...
            <TextInput
              ...
              placeholder="مثال: BUS001" placeholderTextColor={colors.textTertiary} textAlign="right"
            />

            <Text style={styles.label}>رقم اللوحة *</Text>
            <TextInput
              style={styles.modalInput} value={form.plateNumber} onChangeText={(v) => setField('plateNumber', v)}
              placeholder="رقم اللوحة" placeholderTextColor={colors.textTertiary} textAlign="right"
            />

            <Text style={styles.label}>الماركة</Text>
            <TextInput
              style={styles.modalInput} value={form.brand} onChangeText={(v) => setField('brand', v)}
              placeholder="مثال: Toyota" placeholderTextColor={colors.textTertiary} textAlign="right"
            />

            <Text style={styles.label}>الموديل</Text>
            <TextInput
              style={styles.modalInput} value={form.modelName} onChangeText={(v) => setField('modelName', v)}
              placeholder="موديل الحافلة" placeholderTextColor={colors.textTertiary} textAlign="right"
            />

            <Text style={styles.label}>السنة</Text>
            <TextInput
              style={styles.modalInput} value={form.year} onChangeText={(v) => setField('year', v)}
              placeholder="مثال: 2024" placeholderTextColor={colors.textTertiary} textAlign="right" keyboardType="numeric"
            />

            <Text style={styles.label}>السعة</Text>
            <TextInput
              style={styles.modalInput} value={form.capacity} onChangeText={(v) => setField('capacity', v)}
              placeholder="عدد المقاعد" placeholderTextColor={colors.textTertiary} textAlign="right" keyboardType="numeric"
            />
                ...
                <View style={styles.row}>
                  ...
                  <View style={{ flex: 1, marginHorizontal: spacing.md }}>
                    ...
                    <Text style={[typography.bodySmall, { color: colors.textSecondary }]}>
                      {item.plateNumber} | {item.brand} {item.modelName}
                    </Text>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginTop: spacing.xs }}>
                      ...
                      {item.driverId?.fullName && (
                        <Text style={[typography.bodySmall, { color: colors.textTertiary }]}>| السائق: {item.driverId.fullName}</Text>
                      )}
                    </View>
                  </View>
