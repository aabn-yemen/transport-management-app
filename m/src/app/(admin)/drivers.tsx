...
export default function DriversScreen() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      Alert.alert('خطأ', err?.response?.data?.message || 'فشل إضافة السائق');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      setForm(emptyForm);
      refetch();
      Alert.alert('تم', 'تم تعديل بيانات السائق بنجاح');
    } catch (err: any) {
      Alert.alert('خطأ', err?.response?.data?.message || 'فشل تعديل بيانات السائق');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = (item: any) => {
    Alert.alert('حذف سائق', `هل أنت متأكد من حذف "${item.fullName}"؟\nلا يمكن التراجع عن هذا الإجراء.`, [
      { text: 'إلغاء', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await driversApi.delete(item._id || item.id);
            refetch();
            Alert.alert('تم', 'تم حذف السائق بنجاح');
          } catch (err: any) {
            Alert.alert('خطأ', err?.response?.data?.message || 'فشل حذف السائق');
          }
        },
      },
    ]);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
        ...
        <View style={[styles.modalContent, { width: modalWidth, alignSelf: 'center' }]}>
          ...
          <TextInput
            ...
            placeholder="اسم السائق الكامل" placeholderTextColor={colors.textTertiary} textAlign="right"
          />
          ...
          <TextInput
            style={styles.modalInput} value={form.nationalId} onChangeText={(v) => setField('nationalId', v)}
            placeholder="رقم الهوية الوطنية" placeholderTextColor={colors.textTertiary} textAlign="right" keyboardType="number-pad"
          />

          <Text style={styles.label}>رقم رخصة القيادة *</Text>
          <TextInput
            style={styles.modalInput} value={form.licenseNumber} onChangeText={(v) => setField('licenseNumber', v)}
            placeholder="رقم الرخصة" placeholderTextColor={colors.textTertiary} textAlign="right"
          />

          <Text style={styles.label}>تاريخ انتهاء الرخصة *</Text>
          <TextInput
            style={styles.modalInput} value={form.licenseExpiry} onChangeText={(v) => setField('licenseExpiry', v)}
            placeholder="YYYY-MM-DD" placeholderTextColor={colors.textTertiary} textAlign="right"
          />

          <Text style={styles.label}>العنوان *</Text>
          <TextInput
            style={styles.modalInput} value={form.address} onChangeText={(v) => setField('address', v)}
            placeholder="العنوان" placeholderTextColor={colors.textTertiary} textAlign="right"
          />

          <Text style={styles.label}>تاريخ التوظيف *</Text>
          <TextInput
            style={styles.modalInput} value={form.employmentDate} onChangeText={(v) => setField('employmentDate', v)}
            placeholder="YYYY-MM-DD" placeholderTextColor={colors.textTertiary} textAlign="right"
          />

          <Text style={styles.label}>الراتب (ريال) *</Text>
          <TextInput
            style={styles.modalInput} value={form.salary} onChangeText={(v) => setField('salary', v)}
            placeholder="الراتب" placeholderTextColor={colors.textTertiary} textAlign="right" keyboardType="numeric"
          />

          <Text style={styles.label}>رقم السائق</Text>
          <TextInput
            style={styles.modalInput} value={form.driverNumber} onChangeText={(v) => setField('driverNumber', v)}
            placeholder="اتركه فارغاً للإنشاء التلقائي" placeholderTextColor={colors.textTertiary} textAlign="right"
          />

          <Text style={styles.label}>رقم الحافلة</Text>
          <TextInput
            style={styles.modalInput} value={form.busNumber} onChangeText={(v) => setField('busNumber', v)}
            placeholder="رقم الحافلة (اختياري)" placeholderTextColor={colors.textTertiary} textAlign="right"
          />

          <Text style={styles.label}>ملاحظات</Text>
          <TextInput
            style={[styles.modalInput, { height: 60 }]} value={form.notes} onChangeText={(v) => setField('notes', v)}
            placeholder="ملاحظات (اختياري)" placeholderTextColor={colors.textTertiary} textAlign="right" multiline
          />
          ...
          {title.includes('تعديل') && (
            <>
              ...
              <TextInput
                ...
                placeholder="اتركها فارغة للاحتفاظ بكلمة المرور" placeholderTextColor={colors.textTertiary} textAlign="right" secureTextEntry
              />
            </>
          )}
          </ScrollView>
                ...
                <View style={styles.row}>
                  ...
                  <View style={{ flex: 1, marginHorizontal: spacing.md }}>
                    ...
                    <Text style={[typography.bodySmall, { color: colors.textSecondary }]}>
                      {item.driverNumber} | {item.busId?.busNumber ?? '--'}
                    </Text>
