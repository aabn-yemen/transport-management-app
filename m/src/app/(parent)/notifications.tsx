...
export default function ParentNotifications() {
  ...
  const { data: notifications, isLoading, refetch } = useGetAll<any[]>(['parent-notifications'], notificationsApi.getAll);
  const items = notifications?.length ? notifications : [];

  const handlePress = useCallback(async (item: any) => {
    if (!item.read && item._id) {
      try { await notificationsApi.markRead(item._id); refetch(); } catch {}
    }
  }, [refetch]);
      ...
      {items.length === 0 ? (
        ...
        >
          {items.map((item: any) => {
            ...
            return (
              <TouchableOpacity key={item._id ?? item.id} activeOpacity={0.7} onPress={() => handlePress(item)}>
                <Card style={[{ marginBottom: spacing.sm }, !item.read && { borderColor: colors.primary, borderWidth: 1 }]}>
                  <View style={styles.row}>
                    <View style={[styles.iconBox, { backgroundColor: cfg.color + '15' }]}>
                      <Ionicons name={cfg.icon} size={20} color={cfg.color} />
                    </View>
                    <View style={{ flex: 1, marginHorizontal: spacing.md }}>
                      <Text style={[typography.titleSmall, { color: item.read ? colors.textSecondary : colors.text }]}>{item.title}</Text>
