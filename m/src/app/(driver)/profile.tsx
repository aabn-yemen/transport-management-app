...
export default function DriverProfile() {
  ...
  const { data: driverInfo } = useGetOne<any>(
    ['driver-me'],
    driversApi.getMe,
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={[colors.secondary, colors.secondaryDark]} style={styles.header}>
        ...
        {driverInfo && (
          <Text style={[typography.bodyMedium, { color: colors.textInverse + 'cc', marginTop: spacing.xs }]}>
            {driverInfo.driverNumber} | {driverInfo.busId?.busNumber || 'بدون حافلة'}
          </Text>
        )}
      </LinearGradient>
      ...
      <Card style={styles.infoCard}>
        ...
        <InfoRow icon="call-outline" label="الهاتف" value={user?.phone || '--'} />
        <InfoRow icon="id-card-outline" label="رقم السائق" value={driverInfo?.driverNumber || '--'} />
        <InfoRow icon="car-outline" label="الحافلة" value={driverInfo?.busId?.busNumber || 'غير معيّنة'} />
        <InfoRow icon="location-outline" label="العنوان" value={driverInfo?.address || '--'} />
        <InfoRow icon="star-outline" label="التقييم" value={driverInfo?.rating ? `${driverInfo.rating} / 5` : '--'} />
      </Card>
...
function InfoRow({ icon, label, value }: { icon: keyof typeof Ionicons.glyphMap; label: string; value: string }) {
  return (
    <View style={infoStyles.row}>
      ...
      <Text style={[typography.bodyMedium, { color: colors.text, flex: 1, textAlign: 'left' }]}>{value}</Text>
    </View>
  );
}
